module.exports = function TaxiTrips(pool) {
    ...
    let findTripsByRegNumber = async (reg_number) => { try { return (await pool.query(`select route_name, sum(fare_per_trip)  from trip  join route on trip.trip_region = route.region_id where trip.trip_reg_number = '${reg_number}' group by route_name order by route_name asc`)).rows } catch (error) { `${error}` } };
